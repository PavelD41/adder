// Vercel serverless function для отправки данных админу и боту
export default async function handler(req, res) {
  ...
  try {
    ...
    // Формируем сообщение для бота (для сохранения в базу)
    const botMessageText = `SAVE_ANKETA:${user_id}:${data.fio}:${data.age}:${data.experience}:${data.reason}`;
    ...
    console.log('Sending save message to user:', user_id);
    try {
      ...
      console.log('Save message response:', saveData);
    } catch (error) {
      console.error('Error sending to bot for save:', error);
    }
